...
%! Author:  Elayson Abreu 2022--2026
...
\usepackage[german,english,brazilian,shorthands=off]{babel}
...
\usepackage[brazilian,capitalize,noabbrev]{cleveref}
    \AtBeginDocument{\let\ORIref=\ref \def\ref#1{\cref{#1}}}
\usepackage{fontspec}
\usepackage{listings}
...
\begingroup
    \def\do#1{\catcode\number`#1=12}
    \xdef\mydospecials{\dospecials\relax}
\endgroup
\let\setverb=\mydospecials
\def\verbchar#1{%
...
\def\type#1{\NoCaseChange{\protect\dotype{#1}}}
...
% A opção cmyk foi passada para xcolor.
...
\def\hooksection{}
\pretocmd\inappendix{\clearpage}{}{}
\pretocmd\inannex{\clearpage}{}{}
...
\def\ABNT{Associação Brasileira de Normas Técnicas (ABNT)\doABNT}
% Precisa ser \protected contra \edef.
\protected\def\doABNT{\gdef\ABNT{ABNT}}
\ifdefined\bibsetup
    \appto\bibsetup{\raggedright\interlinepenalty=10000
                    \singlesp\bibitemsep=\baselineskip}
\fi
\appto\textual{
    \def\@evenhead{\abntsmall \firstmarks0\hfil\thepage}
    \def\@oddhead {\abntsmall \botmarks1\hfil\thepage}
    \def\sectionmark   #1{\marks0{Seção \thesection: \unexpanded{#1}}\marks1{}}
...
\noindent\hfil {\Large\ttfamily 1.1}
\newpage
...
\begin{topics}
    \item NBR 14724:2024. Informação e documentação --- Trabalhos acadêmicos --- Apresentação;
...
%    \item NBR 6028:2021. Informação e documentação --- Resumo --- Apresentação \cite{abnt-2021-6028-resumo-resenha-recensao};
    \item NBR 6027:2012. Informação e documentação --- Sumário --- Apresentação;
%    \cite{abnt-2012-6027-sumario};
    \item NBR 10520:2023. Informação e documentação --- Citações em documentos --- Apresentação;
%    \cite{abnt-2023-10520-citacoes};
...
%    \item NBR 5892:2019. Informação e documentação --- Representação e formatos de tempo --- Datas e horas --- Apresentação \cite{abnt-2019-5892-representacao-e-formatos-de-tempo};
    \item NBR 6024:2012. Informação e documentação --- Numeração progressiva das seções de um documento --- Apresentação;
%    \cite{abnt-2012-6024-numeracao-progressiva-das};
    \item IBGE. Normas de apresentação tabular.
%    \cite{ibge-1993-normas-de-apresentacao}.
\end{topics}

A versão mais recente desse manual sempre estará no repositório do Github\footnote{\url{https://github.com/ElaysonAbreu/abntexto}.} (a versão no CTAN\footnote{\url{https://ctan.org/pkg/abntexto}.} não necessariamente é a mais recente).

\subsection{Nota de compatibilidade}

No salto da versão 3.2.1-beta para a 4.0.0-beta, muito do código-fonte foi reescrito; funcionalidades foram adicionadas; comandos obsolesceram. Por consequência, documentos antigos irão certamente quebrar em \emph{layout} e aparência.
...
A partir de agora, use `\abntsmall` ao invés de `\small` para o tamanho menor e uniforme. A partir da versão 1.1 da classe, o significado de `\small` será aquele definido na classe `article`.
...
O arquivo `abntexto-exemplo.tex`, disponível no diretório da classe no CTAN e no Github, mostra como elaborar os elementos pré-textuais. Além disso, escritores de classe podem querer usar a classe `abntexto-uece.cls`\footnote{\url{https://ctan.org/pkg/abntexto-uece}.} como exemplo para elaboração de suas classes.

\section{Fonte tipográfica}

A NBR~14\,724:2024 não especifica o tipo de fonte que deve ser usado no documento. Se você precisa de uma fonte \emph{semelhante} a Times New Roman, você pode usar o exemplo em \ref{cd:inicio}. Se você precisa da Times New Roman verdadeira, é necessário, primeiro, estar no \lualatex\ ou no \xelatex, segundo, possuir um sistema operacional que contenha a referida fonte (Windows, por exemplo), terceiro, carregar `fontspec` e, por fim, escrever:
\begin{xp}
    \documentclass{abntexto}
    \usepackage{fontspec}     % Disponibiliza \setmainfont.
    \usepackage{unicode-math} % Disponibiliza \setmathfont.
...
Se você precisa da verdadeira fonte Arial, basta substituir \enquote{Times New Roman}, no exemplo acima, por \enquote{Arial} e substituir \enquote{XITSMath-Regular.otf} por \enquote{FiraMath-Regular.otf} para usufruir de uma fonte matemática compatível com Arial.

Se o usuário final não tem escolha a não usar o \pdflatex\ na compilação, ele pode definir um preâmbulo semelhante ao abaixo para carregar as fontes New TX, semelhantes a Times New Roman:
\begin{xp}
    \documentclass{abntexto}
    % É carregado automaticamente desde 2018:
    % https://tex.stackexchange.com/a/641193/383003
    ...
    \end{document}
\end{xp}
O pacote `newtx` possui muitas opções de personalização, veja o manual no CTAN\footnote{\url{https://ctan.org/pkg/newtx}.}.

Os pacotes `times`, `txfonts` e `mathptmx` são considerados obsoletos.
...
A classe `article`, na qual se baseia o `abntexto`, fornece os seletores habituais `\tiny`, `\footnotesize`, `\Large` etc. No entanto, o `abntexto` se limitou a definir apenas dois: `\normalsize` para o tamanho padrão do texto e \^`\abntsmall` para satisfazer a exigência da NBR~14\,724 acerca do tamanho menor e uniforme: \Enquote{Recomenda-se, quando o trabalho for digitado, a fonte tamanho 12 para todo o texto, inclusive a
...
Existem dois comandos que configuram o \emph{layout} da página: \^`\pretextual` e \^`\textual`. O primeiro é iniciado automaticamente durante `\begin{document}`, invoca \^`\onesidelayout` e remove a numeração de página. O segundo, `\textual`, também é executado automaticamente pelo primeiro `\section` numerado (não prefixado com `\nonum`), invoca \^`\twosidelayout` e ativa a numeração de página.
...
Os trabalhos acadêmicos podem vir a ser disponiblizados num repositório \emph{online}. Nesse caso, o usuário pode querer reconfigurar as margens de forma a se ter uma leitura mais confortável em dispositivos eletrônicos. Para esse propósito foi criado o comando \^`\useeletroniclayout`, apesar dessa configuração não estar prevista na ABNT. O `\useeletroniclayout` centraliza o corpo do texto configurando 2,5~cm nas margens esquerda e direita da página. Veja um exemplo de uso em \ref{cd:eletroniclayout}:

\legend{code}{Uso de \type{\\useeletroniclayout}}
...
\begin{lst}
    ...
    \useeletroniclayout
    \begin{document}
        Corpo do documento com margens esquerda e direita valendo 2,5 cm.
    \end{document}
\end{lst}

Outros comandos de estilo de página tais como `\pagestyle`{\ttfamily \{}{\ttfamily headings, empty, myheadings, plain}{\ttfamily \}}, herdados da classe `article`, não fazem sentido no `abntexto`. Ao invés de usá-los, é aconselhável redefinir `\textual`, `\pretextual` ou ambos.
...
De acordo com a NBR~14\,724 da \textcite[p.~8]{abnt-2024-14724-trabalhos-academicos}: \enquote{O trabalho acadêmico não pode ser dividido em capítulos; deve ser organizado em seções [\dots]}.

O usuário pode querer utilizar capítulos no trabalho, apesar de a unidade secional `\section` ser suficiente para um trabalho acadêmico. Para esse propósito, o `abntexto` disponibiliza a instrução \^`\chapter` para a utilização de capítulos no documento, porém não em conformidade com a NBR~14\,724:2024, mas no estilo da classe `book`. Para ativá-los é preciso chamar a instrução \^`\usechapters` no preâmbulo.
...
É importante lembrar que, de acordo com NBR~6024:2012, \enquote{deve-se limitar a numeração progressiva até a seção quinária} \cite[p.~2]{abnt-2012-6024-numeracao-progressiva-das} mesmo que `\usechapters` e `\useparts` permitam até sete níveis secionais em um documento.

Os níveis secionais `\section` e `\chapter` podem ser prefixados com \^`\nonum` que remove a numeração do título: `\nonum\section`. Os títulos prefixados com `\nonum` serão centralizados
em conformidade com a NBR~14\,724: \Enquote{Os títulos sem indicativo numérico --- errata, agradecimentos, lista de ilustrações, lista de tabelas, lista
de abreviaturas e siglas, lista de símbolos, resumos, sumário, referências, glossário, apêndice(s),
anexo(s) e índice(s) --- devem ser centralizados \cite[p.~10]{abnt-2024-14724-trabalhos-academicos}}
Veja um exemplo de uso de `\nonum` em \ref{cd:nonum}.
...
As unidades secionais supracitadas também podem ser prefixadas com \^`\notoc` que instrui o título a não ingressar no Sumário. Além disso, é possível usar ambos os prefixos ao mesmo tempo, conforme mostrado em \ref{cd:nonum}.

É importante esclarecer que `\nonum\section` e `\nonum\notoc\section` fazem sentido, mas `\notoc\section`, provavelmente não, porque todos os títulos numerados devem ingressar ao sumário. Para ser mais exato, `\notoc\section` produz um título numerado que não ingressa ao sumário nem a lista de marcadores do leitor de PDF.
...
\subsection{Seções quaternárias e quinárias}

Por padrão não há tratamento tipográfico de seções quaternárias e quinárias no `abntexto`, ou seja, `\def\paragraphfont{}`, `\def\tocparagraphfont{}`, `\def\subparagraphfont{}` e `\def\tocsubparagraphfont{}`. Mas, se o leitor estiver usando a fonte XITS no \lualatex\ com `fontspec`, por exemplo, uma formatação pode ser atribuída conforme \ref{cd:quaternárias}. Tivemos que usar `\addfontfeatures{FakeSlant=.2}` ao invés de `\itshape` neste caso porque a fonte XITS não possui versão Small Caps Italic/Slanted.

\legend{code}{Seções quaternárias e quinárias}
\src\me
\label{cd:quaternárias}
\begin{lst}
    \documentclass{abntexto}
    ...
    \def\paragraphfont      {\scshape}
    \def\subparagraphfont   {\scshape\addfontfeatures{FakeSlant=.2}}
    \def\tocparagraphfont   {\scshape}
    \def\tocsubparagraphfont{\scshape\addfontfeatures{FakeSlant=.2}}
...
\subsection{Seções no anverso}

A NBR~14\,724 da \textcite[p.~10]{abnt-2024-14724-trabalhos-academicos} diz que \enquote{para trabalhos apresentados em anverso e verso, os títulos das seções primárias devem começar em página ímpar (anverso). Para trabalhos apresentados apenas no anverso, os títulos das seções primárias devem começar em nova página.}.

O comando de quebra de página em `\part`, `\chapter` e `\section`, nas âncoras \^`\hookpart`, \^`\hookchapter` e \^`\hooksection`, é `\clearpage`:
\begin{xp}
    \def\hookpart{\clearpage}
    \def\hookchapter{\clearpage}
    \def\hooksection{\clearpage}
\end{xp}
O `\clearpage`, todavia, apenas inicia uma nova página, seja ela no verso ou no anverso. Tendo isso em vista, a fim de seguir a recomendação acima, o usuário final pode criar o comando `\nextoddphysicalpage`\footnote{Comando criado com base numa resposta em \url{https://tex.stackexchange.com/a/762186/383003}.} e redefinir as âncoras acima:
\begin{xp}
    \def\nextoddphysicalpage{%
        \clearpage
    ...
    \def\hooksection{\nextoddphysicalpage}
\end{xp}
É necessário ter o TeX Live 2021 ou superior para que o `\ReadonlyShipoutCounter` esteja disponível. Tal modificação não pode ser anexada ao código-fonte da classe porque isso causaria uma incompatibilidade com documentos antigos.
...
A partir da versão 4.0.0-beta, o `abntexto` passou a suportar objetos flutuantes. Por exemplo, em
\begin{xp}
...
A `<especificação>` padrão é `here` caso nenhuma seja explicitamente assumida, mas o usuário pode habilitar a flutuação de objetos digitando, por exemplo, \^`\setplacepos``{tbp}` no preâmbulo do documento.

\subsection{Criando áreas de legenda}

Até a versão 3.2.1-beta, o `abntexto` fornecia \^`\definelegendplace` para criação de novos espaços de legenda sob a sintaxe
\begin{xp}
    \definelegendplace{<tipo>}{<nome>}{<ref rótulo>}{<extensão>}
\end{xp}
Na versão 4.0.0-beta, no entanto, o terceiro argumento foi excluído em favor do suporte mais extensivo do pacote `cleveref.sty` (não carregado pela classe). Veja o \ref{app:cleveref} para mais detalhes. A partir de agora, a nova sintaxe é
\begin{xp}
...
\begin{lst}
    ...
    Ver Figura~\ref{fig:teste1} e Figura~\ref{fig:teste2} em Figura~\ref{fig:duas-figuras}.
\end{lst}
...
\begin{place}\vbox\bgroup
    ...
    \parindent=1.5cm Ver Figura~\ORIref{fig:teste1} e Figura~\ORIref{fig:teste2} em Figura~\ORIref{fig:duas-figuras}.
\egroup\end{place}
...
Também existe o comando \^`\subnote` que funciona de maneira similar a `\subsrc`.
...
Note que, `\legend` precisa ser chamado, do contrário, as `\sublegend`s não seriam impressas porque elas precisam conhecer o valor de `\savedplace`.
...
\begin{lst}
    ...
    Ver Tabela~\ref{tab1}, Figura~\ref{fig1} e Figura~\ref{fig2}.
\end{lst}

\src{\me}
\begin{place}\vbox\bgroup
    \legend{output}{Resultado do \ref{cd:multialt}}
    \label{out:alt}
    \begin{multiplace}
        \sublegend[table]{Tabela de exemplo} \label{tab1}
        \subsrc{\me}
    ...
    \parindent=1.5cm Ver Tabela~\ORIref{tab1}, Figura~\ORIref{fig1} e Figura~\ORIref{fig2}.
\egroup\end{place}

\subsection{Tabelas que se partem entre páginas}

No momento não existe suporte para legendar tabelas e outras estruturas que se partem entre páginas. Se o leitor tiver de usar `longtable.sty` ou `tabularray.sty` (este último é mais flexível), será preciso legendar estruturas tabulares com as ferramentas fornecidas por esses pacotes.

\subsection{Objetos flutuantes e não-flutuantes na mesma página}

Ao compilarmos o \ref{cd:float-nonfloat} notamos que as figuras são impressas como deveriam, mas há um problema: a contagem está fora de ordem. Esse problema pode ser impedido evitando-se inserir objetos flutuantes e não-flutuantes numa mesma página.

\legend{code}{Um objeto flutuante e um não-flutuante na mesma página}
\label{cd:float-nonfloat}
\src{\me}
\begin{lst}
    \documentclass{abntexto}
    \begin{document}
...
A definição de `\bibsetup`\footnote{Esse comando só tem efeito em conjunto com o pacote `biblatex`.} encontra-se em `biblatex.def` (acesso em 28/04/2026):
\begin{xp}
    \newcommand*{\bibsetup}{%
        ...
        \biburlsetup}
\end{xp}
A redefinição em `abnt.bbx` (acesso em 28/04/2026) é:
\begin{xp}
    \appto{\bibsetup}{%
...
\end{xp}
De acordo com a NBR~6023: \Enquote{As referências devem ser elaboradas em espaço simples, alinhadas à margem esquerda do texto e separadas entre si por uma linha em branco de espaço simples. Quando aparecerem em notas de rodapé, devem ser alinhadas à margem esquerda do texto e, a partir da segunda linha da mesma referência, abaixo da primeira letra da primeira palavra, de forma a destacar o expoente e sem espaço entre elas \cite[p.~5]{abnt-2018-6023-referencias}.}
No intuito de adequar-se a referida recomendação, o usuário final pode redefinir `\bibsetup` da seguinte forma:
\begin{xp}
    \ifdefined\bibsetup
        \appto\bibsetup{\raggedright\interlinepenalty=10000\relax
                        \singlesp\bibitemsep=\baselineskip}
    \fi
\end{xp}
O `\interlinepenalty=10000` vai impedir quebras de página nas entradas bibliográficas completamente (a citação acima não toca nessa questão); `\singlesp` deve preceder `\bibitemsep` porque este precisa ter `\baselineskip` simples. Tal redefinição não pode ser incorporada ao código-fonte da classe porque afetaria a compatibilidade com documentos antigos.

\section{Citações}

Visando seguir a norma NBR~10520:2023, são fornecidos `\enquote` do pacote `csquotes`, carregado pela classe, e \^`\Enquote`. Para inserir aspas duplas em citações diretas, de até três linhas, use `\enquote{<texto>} \cite{<chave>}`. Porém, se tratando de citações diretas, com mais de três linhas, de parágrafo único, alterne para `\Enquote{<texto> \cite{<chave>}.}`.
...
Até a versão 3.2.1-beta, era necessário ajustar a largura dos rótulos no Sumário manualmente redefinindo os comandos \^`\appendixlabelwidth` e \^`\annexlabelwidth`. Todavia, a partir da versão 4.0.0-beta, eles se ajustam automaticamente com o auxílio das novas macroinstruções \^`\eqbox` e \^`\eqboxsize`.
...
\begin{xp}
    {\nohyph Parágrafo curto ou título\par} % Esse \par é necessário para
                                            ...
                                            % entre em ação.
\end{xp}
Numa `minipage`, o `\par` final não é necessário: há um em `\end{minipage}`.
...
Percebe-se que a chave `brazil` não é reconhecida pelo `cleveref`: use `brazilian`. Além disso, ele deve ser carregado depois do `hyperref.sty`. Veja outras funcionalidades no manual do `cleveref`.

\appendix{Lista de Abreviaturas}

Elemento opcional. Deve constar antes do sumário. O `glossaries-extra.sty` \cite{glossaries-extra-package} consegue criar uma Lista de Abreviaturas, conforme \ref{cd:abbrev}.
...
Elemento opcional. Deve constar antes do sumário. O `glossaries-extra.sty` \cite{glossaries-extra-package} também consegue criar uma Lista de Siglas, conforme \ref{cd:acro}.
...
Elemento opcional. Deve constar antes do sumário. O `glossaries-extra.sty` \cite{glossaries-extra-package} também consegue criar uma Lista de Símbolos, conforme \ref{cd:simbolos}.
...
Elemento opcional. Deve constar logo depois das Referências. Você pode criar glossários com `glossaries-extra` \cite{glossaries-extra-package}, como em \ref{cd:gloss}.
...
\def\sectionmark   #1{\marks0{\noexpand\ref{ap:codigo-fonte}, Seção \thesection: \unexpanded{#1}}\marks1{}}
\def\subsectionmark#1{\marks1{\noexpand\ref{ap:codigo-fonte}, Subseção \thesubsection: \unexpanded{#1}}}
