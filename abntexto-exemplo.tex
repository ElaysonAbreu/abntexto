...
\usepackage{lipsum}
\usepackage[english,brazilian,shorthands=off]{babel}
...
\def \Titulo{Classe {\ttfamily abntexto} para \LaTeX\ versão 1.1: um exemplo de uso}
...
\long\def\note#1{\abntextonote{#1}}

\def\Centro{\noindent\hfil}
\def\Direita{\noindent\hfill}
\def\me{Elaboração própria.}
\appto\bibfont{\raggedright\interlinepenalty=10000 \singlesp\bibitemsep=\baselineskip}
\hypersetup{
    pdfauthor   = \Autor,
    pdftitle    = \Titulo,
...
% De acordo com a NBR 14724, “os elementos pré-textuais devem iniciar no
% anverso da folha, com exceção dos dados internacionais de catalogação na publicação,
% que devem constar no verso da folha de rosto. Recomenda-se que os elementos textuais e
% pós-textuais sejam apresentados no anverso e verso das folhas (ABNT, 2024, p. 10)”.
...
{\selectlanguage{english}Keywords: Keyword A. Keyword B. Keyword C.\par}
\newpage

% LISTA DE FIGURAS (OPCIONAL), TABELAS (OPCIONAL), SUMÁRIO
% ================================================

\nonum\notoc\section{\listfigurename}
\makelof
\newpage\null\newpage

\nonum\notoc\section{\listtablename}
\makelot
\newpage\null\newpage
...
\section{Uma seção primária}
\subsection{Uma seção secundária}
\subsubsection{Uma seção terciária}
\paragraph{Uma seção quaternária}
\subparagraph{Uma seção quinária}
...
\begin{multiplace}
    ...
    \begin{subplace}
        ...
        \end{tikzpicture}
    \end{subplace}
    \sublegend[c]{A estrutura química do etanol}
    \label{q:etanol}
    \subsrc\me
    \hfil
    \begin{subplace}
    ...
    \end{subplace}
    \sublegend[circ]{\raggedright Fonte de tensão $V$ e dois resistores $R_1$ e $R_2$ ligados em paralelo.}
    \label{circ:am}
    \subsrc\me
    \vadjust{\nobreak}\\[\baselineskip] \null \hfil
    \begin{subplace}
